import ListSaver from './ListSaver.js';
...
const suite = new Test0({ _name: 'ListSaver' });
...
suite
    .add("save() writes a JSON array to the path", async t => {
        ...
        const saver = new ListSaver({ path: '/data/col.json' });
    ...
    .add("load() appends items from the JSON array", async t => {
        ...
        const list = new List8();
        const saver = new ListSaver({ path: '/data/load.json' });
    ...
    .add("load() uses item_class for instantiation", async t => {
        ...
        const list = new List8();
        const saver = new ListSaver({ path: '/data/cls.json', item_class: MyItem });
        await saver.load(list);
    ...
    .add("load() with missing file — no-op", async t => {
        StubSocket._clear();
        const list = new List8();
        const saver = new ListSaver({ path: '/data/missing.json' });
    ...
    .add("save() then load() round-trip", async t => {
        ...
        const saver = new ListSaver({ path: '/data/rt.json' });
    ...
    .add("save() with empty list writes empty array", async t => {
        StubSocket._clear();
        const list = new List8();
        const saver = new ListSaver({ path: '/data/empty.json' });
        await saver.save(list);
    ...
    .add("delete() removes the file", async t => {
        ...
        const saver = new ListSaver({ path: '/data/del.json' });
        await saver.delete();
    ...
    .add("auto-save pattern: on('add') triggers save", async t => {
        StubSocket._clear();
        const list = new List8();
        const saver = new ListSaver({ path: '/data/auto.json' });
