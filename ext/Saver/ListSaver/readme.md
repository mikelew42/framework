# ListSaver — Persist a List to One JSON File
...
| All items in one file | `ListSaver` on the List |
| Named independent Items | `Store` |

`ListSaver` is for collections that are always loaded and saved together: a todo list, a set of settings entries, a small database table.

## Usage

```js
import { List, ListSaver } from "/app.js";
import MyItem from "./MyItem.js";

const todos = new List();
const saver = new ListSaver({ path: '/data/todos.json', item_class: MyItem });
...
```js
const saver = new ListSaver({ path, item_class? });
...
- Unlike `FileSaver`, `ListSaver` takes a `list` as its target, not an `item`. Attach it directly to the list rather than via `item.saver`.
